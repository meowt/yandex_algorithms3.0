...
//на них буквами латинского алфавита. Теперь-то ему будет чем заняться долгими вечерами,
...
//таких, что следующая буква в строке является следующей по алфавиту. Например, хорошесть строки
...
func main() {
	...
	fmt.Println(calc(slc))
}

func calc(slc []int) (res int) {
	for i := 0; i < len(slc)-1; i++ {
		res += int(math.Min(float64(slc[i]), float64(slc[i+1])))
	}
	return
}
